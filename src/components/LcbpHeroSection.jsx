  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at var(--mx) var(--my), rgba(251,146,60,0.09), transparent 20%), radial-gradient(circle at calc(100% - var(--mx)) calc(var(--my) * 0.8), rgba(56,189,248,0.08), transparent 24%)",
        }}
      />
      ...
      <style>{`
        @keyframes lcbpHeroReveal {
          from { opacity: 0; transform: translateY(-24px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        .lcbp-hero__copy {
          animation: lcbpHeroReveal 0.82s cubic-bezier(0.22, 1, 0.36, 1) both;
        }
      `}</style>
    </section>
  );
