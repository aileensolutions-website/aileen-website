// ==============================
// ✅ CONFIG: ปรับเวลาตรงนี้
// ==============================

// เวลาแช่ไอคอน (เปลี่ยนกลุ่มทุกกี่ ms)  ✅ เพิ่มให้แช่นานขึ้น
const GROUP_DURATION = 3200;

// เวลาอนิเมชันเส้น (แยกออกจากไอคอน เพื่อไม่ให้เส้นช้าเกินไป)
const LINE_DURATION = 1800;

// กันพลาด: ให้เส้นจบก่อนเปลี่ยนกลุ่มเสมอ (เผื่อคุณไปปรับค่าในอนาคต)
const SAFE_LINE_DURATION = Math.min(LINE_DURATION, GROUP_DURATION - 200);
...
function setPaths() {
  ...
  p67.setAttribute("d", `M ${i(s67.x)} ${i(s67.y)} L ${i(e67.x)} ${i(e67.y)}`);
  ...
  pUp.setAttribute("d", `M ${i(sUp.x)} ${i(sUp.y)} L ${i(eUp.x)} ${i(eUp.y)}`);
  ...
  g2_h1.setAttribute("d", `M ${i(h1s.x)} ${i(h1s.y)} L ${i(h1e.x)} ${i(h1e.y)}`);
  ...
  g2_v.setAttribute("d", `M ${i(vs.x)} ${i(vs.y)} L ${i(ve.x)} ${i(ve.y)}`);
  ...
  g2_h2.setAttribute("d", `M ${i(h2s.x)} ${i(h2s.y)} L ${i(h2e.x)} ${i(h2e.y)}`);
...
function setDurations_g0() {
  ...
  const drawTotal = SAFE_LINE_DURATION * drawShare,
    eraseTotal = SAFE_LINE_DURATION * eraseShare;

  lines.style.setProperty("--d-ab", (drawTotal * (L_ab / Ltot)) / 1000 + "s");
  lines.style.setProperty("--d-bc", (drawTotal * (L_bc / Ltot)) / 1000 + "s");
  lines.style.setProperty("--d-v2", (drawTotal * (L_v2 / Ltot)) / 1000 + "s");

  lines.style.setProperty("--e-ab", (eraseTotal * (L_ab / Ltot)) / 1000 + "s");
  lines.style.setProperty("--e-bc", (eraseTotal * (L_bc / Ltot)) / 1000 + "s");
  lines.style.setProperty("--e-v2", (eraseTotal * (L_v2 / Ltot)) / 1000 + "s");
}
...
function setDurations_g1() {
  ...
  const pause =
    parseFloat(
      getComputedStyle(document.documentElement).getPropertyValue("--pause7")
    ) || 0.12;
...
function setDurations_g2() {
  ...
  const drawTotal = SAFE_LINE_DURATION * drawShare;
  const eraseTotal = SAFE_LINE_DURATION * eraseShare;
  ...
  // ✅ ลบแบบ split ตามเดิม
  const drawEndSec = Math.max(d_h1, d_h12 + d_v + d_h2) / 1000;

  const off_h1 = drawEndSec;
  const off_h12 = drawEndSec;
  const off_v = off_h12 + e_h12 / 1000;
  const off_h2 = off_v + e_v / 1000;
...
// เดิมของคุณมี loop ที่ setPaths ทุกเฟรม (หนักหน่อยแต่คงไว้ตามเดิม)
function loop() {
  setPaths();
